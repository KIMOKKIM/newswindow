...
if (t.includes('function nwFetchNetworkHeadlinesWithTimeout')) {
...
function setLatestTop5FetchErrorState(sec, isTimeout) {
    ...
    btn.setAttribute('aria-label', '���신 기사 다시 불러오기');
...
/** Headlines: /api/home/headlines first, then public latest hero;4s each; independent of ads/home bundle */
...
if (!t.includes(MARK)) throw new Error('marker missing');
t = t.replace(MARK, INSERT + MARK, 1);

const OLD_PAYLOAD = `function nwApplyMainFromHomePayload(payload) {
    if (!payload) return;
    if (payload.ads) {
...
const NEW_PAYLOAD = `function nwApplyMainFromHomePayload(payload) {
    if (!payload) return;
    nwApplyMainArticlesArray(payload.latestArticles || []);
...
if (!t.includes(OLD_PAYLOAD)) throw new Error('old payload missing');
t = t.replace(OLD_PAYLOAD, NEW_PAYLOAD, 1);

const START = 'function nwFetchMainHomeBundle() {';
const END = "\n\ndocument.addEventListener('DOMContentLoaded', function () {";
const i0 = t.indexOf(START);
const i1 = t.indexOf(END);
if (i0 < 0 || i1 < 0 || i1 <= i0) throw new Error('bundle bounds');

const NEW_BUNDLE = `function nwFetchMainHomeBundle() {
    nwHomePerfEnsureStarted();
    var perf = window.__NW_HOME_PERF__;
...
t = t.slice(0, i0) + NEW_BUNDLE + t.slice(i1);
